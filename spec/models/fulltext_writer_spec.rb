...
describe :fulltext_writer do
  ...
  it "should use a file stream if created with a path option" do
    ...
    @w.add_document(1, {:sample_field => 'A'}, @field_map, @field_infos, @suffix_array_writer, @map_writer)
    @w.finish!
  ...
  it "should add a document" do
    ...
    @w.add_document(1, {:sample_field => 'A'}, @field_map, @field_infos, @suffix_array_writer, @map_writer)
    @w.finish!
    @w.data.should == "\n\000\000\000\001\000\000\000\001\000\000\000A\000\000"
  end
  
  it "should create a document mapping when the document is added" do
    @w = FTSearch::FulltextWriter.new
    @w.add_document(1, {:uri => 'X', :sample_field => 'Z'}, @field_map, @field_infos, @suffix_array_writer, @map_writer)
    @map_writer.instance_variable_get('@uri_tbl')[1].should == 'X'
    @map_writer.instance_variable_get('@field_arr')[0].should == [4, 1, 0, 1] # offset, index, field_index, size
  end
  
  it "should store the field information and write the suffixes when the document is added" do
    @w = FTSearch::FulltextWriter.new
    @w.add_document(1, {:sample_field => 'suffixB suffixA'}, @field_map, @field_infos, @suffix_array_writer, @map_writer)
    @suffix_array_writer.instance_variable_get('@suffixes').should == [12, 20]
  end  
